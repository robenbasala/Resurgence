...
export async function fetchReport(params) {
  const { data } = await apiClient.get("/report", { params, timeout: REPORT_TIMEOUT_MS });
  return data;
}

export async function fetchReportMeta() {
  const { data } = await apiClient.get("/report/meta");
  return data;
}

export async function fetchCellDetails({ mrNumber, date }) {
  const { data } = await apiClient.get("/report/cell-details", {
...
export async function fetchCellAnnotations(params) {
  const { data } = await apiClient.get("/report/cell-annotations", { params, timeout: REPORT_TIMEOUT_MS });
  return data;
}

export async function saveCellAnnotation(payload) {
  const { data } = await apiClient.put("/report/cell-annotations", payload, { timeout: DEFAULT_TIMEOUT_MS });
  return data;
}
